...
The scaffold lands exactly this shape. Everything not listed (decision records, operations, quality, generated knowledge, extra `docs/workflows/`) is intentionally omitted and created only when a real need appears — then declare it in `.ai/harness.json`.
...
|   |-- tasks/
|   |   |-- README.md               # board conventions; per-agent boards created on demand
...
The `knowledge.index`, `knowledge.architecture`, and `knowledge.product` paths are core. `knowledge.plans`, `knowledge.agents`, and `knowledge.tasks` are declared by default in the scaffolded manifest. `operations`, `quality`, and `generated` are omitted; declare a path only when the project creates that branch, and keep the routing index consistent with intentional omissions.
